...
#################################
#  Lambda : rest-api-fims_service_lambda
#################################

resource "aws_lambda_function" "api_fims_service_lambda" {
...
#################################
#  Lambda : worker-fims_service_lambda
#################################
...
#  API Gateway
##############################
resource "aws_api_gateway_rest_api" "fims_service_api" {
  name        = "${local.env_composite_name}"
}

resource "aws_api_gateway_resource" "fims_service_api_resource" {
  rest_api_id = "${aws_api_gateway_rest_api.fims_service_api.id}"
  parent_id   = "${aws_api_gateway_rest_api.fims_service_api.root_resource_id}"
  path_part   = "{proxy+}"
}

resource "aws_api_gateway_method" "fims_service_api_method" {
  rest_api_id   = "${aws_api_gateway_rest_api.fims_service_api.id}"
...
resource "aws_api_gateway_integration" "fims_service_api_method-integration" {
  rest_api_id             = "${aws_api_gateway_rest_api.fims_service_api.id}"
  resource_id             = "${aws_api_gateway_resource.fims_service_api_resource.id}"
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  function_name = "${aws_lambda_function.api_fims_service_lambda.arn}"
...
resource "aws_api_gateway_deployment" "fims_service_deployment" {
  depends_on = [
    "aws_api_gateway_method.fims_service_api_method",
    "aws_api_gateway_integration.fims_service_api_method-integration",
  ]
